...
fn test_snapshot_roundtrip_basic() {
    ...
    engine
        ...
        .unwrap();
    engine
        .set("user:2", b"Bob".to_vec(), None, MemoryType::Semantic)
        .unwrap();
    engine
        .set(
            "skill:login",
            b"password_hash".to_vec(),
            None,
...
fn test_snapshot_roundtrip_all_memory_types() {
    ...
    // Verify values
    assert_eq!(
        engine2.get("epi:event1"),
        Some(b"short-lived event".to_vec())
    );
    assert_eq!(engine2.get("sem:fact1"), Some(b"the sky is blue".to_vec()));
    assert_eq!(
        engine2.get("proc:skill1"),
        Some(b"how to ride a bike".to_vec())
    );
...
fn test_snapshot_expired_entries_not_restored() {
    ...
    // Verify
    assert!(
        engine2.get("epi:will_expire").is_none(),
...
fn test_snapshot_multiple_and_restore_latest() {
    ...
    engine1
        ...
        .unwrap();
    engine1
        .set("b", b"2".to_vec(), None, MemoryType::Semantic)
        .unwrap();
    ...
    engine2
        ...
        .unwrap();
    engine2
        .set("c", b"3".to_vec(), None, MemoryType::Semantic)
        .unwrap();
...
fn test_collect_entries() {
    ...
    engine
        ...
        .unwrap();
    engine
        .set("k2", b"v2".to_vec(), Some(600_000), MemoryType::Episodic)
        .unwrap();
    engine
        .set("k3", b"v3".to_vec(), None, MemoryType::Procedural)
        .unwrap();
...
fn test_snapshot_binary_values() {
    ...
    engine
        .set("bin:data", binary_val.clone(), None, MemoryType::Procedural)
        .unwrap();

    // Empty value
    engine
        .set("bin:empty", b"".to_vec(), None, MemoryType::Semantic)
        .unwrap();
...
fn test_snapshot_namespace_keys() {
    ...
    engine
        ...
        .unwrap();
    engine
        .set("session:abc:2", b"s2".to_vec(), None, MemoryType::Episodic)
        .unwrap();
    engine
        .set("session:def:1", b"s3".to_vec(), None, MemoryType::Episodic)
        .unwrap();
    engine
        .set("cache:img:1", b"c1".to_vec(), None, MemoryType::Semantic)
        .unwrap();
    engine
        .set("cache:img:2", b"c2".to_vec(), None, MemoryType::Semantic)
        .unwrap();
...
fn test_snapshot_compression_large_values() {
    ...
    engine
        .set("big:key", large_value.clone(), None, MemoryType::Semantic)
        .unwrap();
    engine
        .set("small:key", b"tiny".to_vec(), None, MemoryType::Semantic)
        .unwrap();
...
fn test_snapshot_no_compression_small_values() {
    ...
    engine
        .set("k1", b"hello".to_vec(), None, MemoryType::Semantic)
        .unwrap();
    engine
        .set("k2", b"world".to_vec(), None, MemoryType::Episodic)
        .unwrap();
...
fn test_snapshot_backward_compat_v1() {
    ...
    assert_eq!(
        engine.get("legacy:key"),
        Some(b"legacy value data".to_vec())
    );
}
